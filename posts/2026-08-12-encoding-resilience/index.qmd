...
subtitle: "Why Per-File Detection Beats Heuristics"
description:  |
  Why binary downloads with per-file encoding detection prevent silent failures: A practical guide to building resilient data pipelines.
author: "Peter Baumgartner"
date: "2026-08-14"
date-modified: last-modified
image: "encoding-problem.webp"
image-alt: "The image features the repair of a digital 'glitch' effect. On the left side, you see a chaotic stream of glowing hexadecimal code, binary digits, and corrupted UTF-8 characters. As it moves to the right, the chaotic stream integrates into the word 'DATA', written in clean, minimalist white sans-serif typography. "
categories: [R, reproducibility, encoding, data-cleaning, workflow, pressfreedom.data]
draft: false
---

## 25 Years of Press Freedom Index Data

Because I had to update the data annually in my pressfreedom while developing [pressfreedom.data](https://github.com/petzi53/pressfreedom.data), an R data package, I learned that writing error-free code is not enough to meet current challenges. While building a pipeline for 24 years of press freedom data, I discovered that even the most evidence-based assumptions can fail if they don't account for the uncertainty of what comes next.

The data I relied on came from [Reporters Without Borders](https://rsf.org/en/index) (RSF). Unfortunately, I couldn't download one clean dataset, but had to stitch together press freedom index data from 2002 to 2026
...
< "The World Press Freedom Index (WPFI) is an annual ranking of 180 countries compiled and published by Reporters Without Borders (RSF) since 2002 based upon the assessment of the non-governmental organization as well as surveys of professionals around the world, of the countries' press freedom records in the previous year. It intends to reflect the degree of freedom that journalists, news organizations, and netizens have in each country, and the efforts made by authorities to respect this freedom. [Wikipedia](https://en.wikipedia.org/w/index.php?title=World_Press_Freedom_Index&oldid=1352761010)"
...
- **Period 1 – From 2002-2012 (2011 is missing)**: The index focused primarily on physical violence, censorship, and media control, including threats against journalists and restrictions on internet access (see [Questionnaire for compiling the 2010 Press Freedom Index](https://rsf.org/sites/default/files/medias/file/2022/04/RSF_Questionnaire_2010_EN.pdf)). The scoring scale was neither reproducible nor metric. The higher the score, the worse the press freedom, with 0 representing the best possible score but no upper limit for the worst situation.
- **Period 2 – From 2013-2021**: The methodology shifted toward measuring pluralism, media independence, and the legislative and infrastructural environments (see [RSF SURVEY 2016](https://rsf.org/sites/default/files/medias/file/2022/04/RSF_Index_Questionnaire_2016_EN.pdf)). This era introduced a scoring scale, in which 100 represents the ideal country and 0 the worst possible scenario.
- **Period 3 – From 2022 onwards**: A major overhaul occurred, moving to a much more granular 118-question questionnaire. This modern approach also evaluates five key contextual components: political, legal, economic, sociocultural, and safety, with each component receiving a score from 0 to 100 (see [World Press Freedom Index: Questionnaire 2022](https://rsf.org/sites/default/files/medias/file/2022/04/RSF_Index_Questionnaire_2022_EN.pdf) and [Methodology used for compiling the World Press Freedom Index 2025](https://rsf.org/en/methodology-used-compiling-world-press-freedom-index-2025?year=2025&data_type=general).
...
#| lst-label: lst-wrong-solution
#| lst-cap: "**My frist (wrong) approach**"
#| eval: FALSE
...
::: {.callout-note style="color: blue;" #nte-wrong-solution-rsf-url}
##### Where does the URL for every year come from? 
...
I knew that [UTF-8](https://datatracker.ietf.org/doc/html/rfc3629) has become the de facto standard for the web and modern software development. In contrast, other encoding schemes, such as ISO-8859-1 (also known as Latin-1), are largely restricted to legacy systems. It therefore seemed natural to me that RSF uses UTF-8 for all files.
...
```{r}
#| label: solution-1
#| lst-label: lst-solution1
#| lst-cap: The first part of the solution
#| eval: FALSE
...
## Second part of the solution {#sec-solution-2}
...
1. With the help of AI (Claude Sonnet 5), I designed a new function named `detect_csv_encoding()`. It takes one argument, filepath, which should be a string representing the path to the file you want to inspect.
2. Then the function guesses the encoding with `guesses <- readr::guess_encoding(filepath)`. The `guess_encoding()` function is from the {**readr**} package (part of the Tidyverse). It reads the file’s raw bytes and compares them against known patterns to determine which encoding was used. It returns a data frame (specifically a tibble) containing columns for the encoding name and the confidence level (0 to 1).
3.  `if (nrow(guesses) == 0) stop(...)` is a safety check. If {**readr**} cannot find any recognizable pattern at all and returns an empty data frame, the function stops with an informative error instead of silently guessing.
4. Since `readr::guess_encoding()` returns multiple possibilities sorted by confidence, we only care about the most likely one. `top_guess <- guesses$encoding[1]` extracts the string from the encoding column of the first row (the one with the highest confidence, the best guess).
5. The first `grepl()` explicitly checks whether the top guess is `UTF-8`, `US-ASCII`, or `ASCII` (all strict subsets of UTF-8; readr sometimes detects pure-ASCII text as `ASCII`). If so, the function returns `"UTF-8"`.
6. The second `grepl()` checks whether the top guess belongs to the Western European Latin-1 family: `ISO-8859-1`, `windows-1252`, or `latin1`. (More details on the regex at the end of this listing.) Even though these three are slightly different, they are often interchangeable for basic Western text. By forcing them all to `"ISO-8859-1"`, the rest of the code can rely on a single, predictable string.
7. If neither branch matched, the function `stop()`s with the actual detected encoding name. This is a deliberate design choice: earlier versions of this function fell through to a silent `return("UTF-8")` default here, which would have hidden the very problem this whole post is about. See the callout below for why this matters.
...
- `(ISO-8859-1|windows-1252|latin1)`: The pipe `|` acts as an OR operator. It checks whether the string is exactly `ISO-8859-1`, `windows-1252`, or `latin1`.
...
::: {.callout-note style="color: blue;" #nte-encoding-resilience-scope}
##### Why only two encodings — and why fail instead of guess?

It's tempting to think this detector should be extended to recognize *every*
possible encoding. But `readr::guess_encoding()` (built on stringi/ICU) already
returns a *ranked, probabilistic* guess across many candidate encodings — it's
not a hard classifier, and widening the regex to match more encoding names
wouldn't make the underlying guess any more certain. It would just add
complexity without addressing the actual risk.

Restricting the detector's *known-good* outcomes to UTF-8 and ISO-8859-1 is
instead a deliberate, evidence-based decision: RSF's files have only ever used
these two encodings across more than two decades of data. Encoding that as
domain knowledge is reasonable.

The part that actually mattered was the *fallback*. My original version treated
"not confidently Latin-1" as "must be UTF-8" — silently defaulting instead of
checking. That's precisely the assumption that broke earlier in this post. The
fixed version above checks for UTF-8 and ISO-8859-1 explicitly and `stop()`s on
anything else (including an empty guess). If RSF ever introduces a third
encoding, or ships a corrupted file, the function now fails loudly at the
moment of detection — instead of mislabeling it and letting bad data slip
downstream.
:::
...
```{r}
#| label: tbl-encoding-demo-1
#| eval: TRUE
#| layout-ncol: 2
#| tbl-cap: "**Guessing the type of encoding**"
#| tbl-subcap: ["rwb2024.csv (UTF-8)", "rwb2025.csv (ISO-8859-1)"]
...
knitr::kable(readr::guess_encoding(file_path_25))
...
detect_csv_encoding(file_path_25)
```
...
coins_metadata <- "title='ctx_ver=Z39.88-2004&rft_val_fmt=info%3aofi%2ffmt%3akev%3amtx%3adc&rft.type=blogPost&rft.title=Encoding%20Resilience%3a%20Why%20Per-File%20Detection%20Beats%20Heuristics&rft.date=2026&rft.source=Thought%20Splinters&rft_id=https://peter-baumgartner.net/posts%2f2026-08-12-encoding-resilience%2findex&rft.language=en&rft.rights=CC%20BY%204.0&rft.description=Why%20binary%20downloads%20with%20per-file%20encoding%20detection%20prevent%20silent%20failures%3a%20A%20practical%20guide%20to%20building%20resilient%20data%20pipelines.%0a&rft.au=Baumgartner%2c%20Peter'"
