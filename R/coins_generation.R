...
#    3. lang / license:   getOption("coins.*")           → filled in
...
build_coins_pairs <- function(meta, file_path = NULL) {
    ...
    if (is.null(meta$lang) || !nzchar(meta$lang)) {
        meta$lang <- getOption("coins.lang", default = NA_character_)
    }

    if (is.null(meta$license) || !nzchar(meta$license)) {
        meta$license <- getOption("coins.license", default = NA_character_)
    }
